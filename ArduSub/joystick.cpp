...
void Sub::transform_manual_control_to_rc_override(int16_t x, int16_t y, int16_t z, int16_t r, uint16_t buttons) {
	...
	float rpyScale = 0.4*g.gain; // Scale -1000-1000 to -400-400 with gain
	float throttleScale = 0.8*g.gain; // Scale 0-1000 to 0-800 with gain
	...
	switch ( get_button(button)->function(shift) ) {
		...
		case JSButton::button_function_t::k_gain_toggle:
			if ( !held ) {
				...
				gcs_send_text_fmt(MAV_SEVERITY_INFO,"#Gain: %2.0f%%",g.gain*100);
				g.gain.notify();
			}
			break;
		case JSButton::button_function_t::k_gain_inc:
			if ( !held ) {
				...
				g.numGainSettings.set_and_save(constrain_int16(g.numGainSettings, 2, 10));
				g.gain = constrain_float(g.gain + (g.maxGain-g.minGain)/(g.numGainSettings-1), g.minGain, g.maxGain);
				gcs_send_text_fmt(MAV_SEVERITY_INFO,"#Gain is %2.0f%%",g.gain*100);
				g.gain.notify();
			}
			break;
